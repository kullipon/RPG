...
void Map::temps(Enemie *teki1, Enemie *teki2, sf::Time &time, sf::Clock &clock, sf::RenderWindow &window)
{
	...
	if (time.asMilliseconds() <= 500)
	{
		if (m_boucle == 0)
		{
			...
			{
				teki1->deplacementAuto(teki1, *this);
			

			}
			if (teki2->get_attacked() == false)
			{
				teki2->deplacementAuto(teki2, *this);
			}
...
void Map::temps(Enemie *teki, sf::Time &time, sf::Clock &clock, sf::RenderWindow &window)
{
